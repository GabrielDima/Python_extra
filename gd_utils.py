...
def save_figure(image_display, plot_dir, plot_name, format_type=[], dpi=500, silent=True,
    ...
    if len(format_type) == 0:
        # Default state.
        image_display.savefig(plot_name + '.pdf', dpi=dpi, bbox_inches='tight', pad_inches=0.1)
        image_display.savefig(plot_name + '.png', dpi=dpi, bbox_inches='tight', pad_inches=0.1)
        image_display.savefig(plot_name + '.eps', dpi=dpi, bbox_inches='tight', pad_inches=0.1)
        image_display.savefig(plot_name + '.jpg', dpi=dpi, bbox_inches='tight', pad_inches=0.1)
        format_type = ['pdf', 'png', 'eps', 'jpg']
    else:
        for i, f_out in enumerate(format_type):
            if f_out == 'pdf' and record is not None:
                image_display.savefig(plot_name + '.' + f_out, dpi=dpi, bbox_inches='tight',
...
def print_out(prt_txt, out_file=None):
    ...
    print(out_string)
    print(out_string, file=info_output_file)
...
def mgn(image, a=(5., 5000.), b=(0., 1.), w=0.3, gamma=3.2, sigma=[2.5, 5, 10, 20, 40], k=0.7):
    ...
    for s in sigma:
        ...
        # sigma_w
        swi = np.sqrt(scipy.ndimage.gaussian_filter((image - bwi) ** 2, s, mode='nearest'))
...
def get_aia_cmap(a_w):
	"""
	Return an instance of the AIA maps for an input string.
	a_w : String identifying the map: 94, 131, 171, 193, 211, 335, 304
	"""    
	...
	for i, wav in enumerate(aia_wavs):
		if a_w == wav:
			return aia_cmaps[i]
